...
class I3D(torch.nn.Module):
    def __init__(self,
        ...
        conv3d_1a_7x7_n112 = Unit3Dpy(
            ...
            padding='SAME')
        # 1st conv-pool
        self.conv3d_1a_7x7_n112 = conv3d_1a_7x7_n112
        self.maxPool3d_2a_3x3_n112 = MaxPool3dTFPadding(
            kernel_size=(1, 3, 3), stride=(1, 2, 2), padding='SAME')
        # conv conv
        conv3d_2b_1x1_n112 = Unit3Dpy(
            out_channels=64,
            in_channels=64,
            kernel_size=(1, 1, 1),
            padding='SAME')
        self.conv3d_2b_1x1_n112 = conv3d_2b_1x1_n112
        conv3d_2c_3x3_n112 = Unit3Dpy(
            out_channels=192,
            in_channels=64,
            kernel_size=(3, 3, 3),
            padding='SAME')
        self.conv3d_2c_3x3_n112 = conv3d_2c_3x3_n112
        self.maxPool3d_3a_3x3_n112 = MaxPool3dTFPadding(
            kernel_size=(1, 3, 3), stride=(1, 2, 2), padding='SAME')

        # Mixed_3b
        self.mixed_3b_n112 = Mixed(192, [64, 96, 128, 16, 32, 32])
        self.mixed_3c_n112 = Mixed(256, [128, 128, 192, 32, 96, 64])

        self.maxPool3d_4a_3x3_n112 = MaxPool3dTFPadding(
            kernel_size=(3, 3, 3), stride=(2, 2, 2), padding='SAME')

        # Mixed 4
        self.mixed_4b_n112 = Mixed(480, [192, 96, 208, 16, 48, 64])
        self.mixed_4c_n112 = Mixed(512, [160, 112, 224, 24, 64, 64])
        self.mixed_4d_n112 = Mixed(512, [128, 128, 256, 24, 64, 64])
        self.mixed_4e_n112 = Mixed(512, [112, 144, 288, 32, 64, 64])
        self.mixed_4f_n112 = Mixed(528, [256, 160, 320, 32, 128, 128])

        self.maxPool3d_5a_2x2_n112 = MaxPool3dTFPadding(
            kernel_size=(2, 2, 2), stride=(2, 2, 2), padding='SAME')

        # Mixed 5
        self.mixed_5b_n112 = Mixed(832, [256, 160, 320, 32, 128, 128])
        self.mixed_5c_n112 = Mixed(832, [384, 192, 384, 48, 128, 128])

        self.avg_pool_n112 = torch.nn.AvgPool3d((2, 4, 4), (1, 1, 1))
        self.dropout_n112 = torch.nn.Dropout(dropout_prob)
        self.conv3d_0c_1x1_n112 = Unit3Dpy(
            in_channels=1024,
            ...
            use_bias=True,
            use_bn=False)
        self.softmax_n112 = torch.nn.Softmax(1)

    def forward(self, inp):
        out = self.conv3d_1a_7x7_n112(inp)
        out = self.maxPool3d_2a_3x3_n112(out)
        out = self.conv3d_2b_1x1_n112(out)
        out = self.conv3d_2c_3x3_n112(out)
        out = self.maxPool3d_3a_3x3_n112(out)
        out = self.mixed_3b_n112(out)
        out = self.mixed_3c_n112(out)
        out = self.maxPool3d_4a_3x3_n112(out)
        out = self.mixed_4b_n112(out)
        out = self.mixed_4c_n112(out)
        out = self.mixed_4d_n112(out)
        out = self.mixed_4e_n112(out)
        
        out = self.mixed_4f_n112(out)
        #out1.shape =  torch.Size([17, 832, 32, 7, 7])
        
        out = self.maxPool3d_5a_2x2_n112(out)
        out = self.mixed_5b_n112(out)
        
        out = self.mixed_5c_n112(out)
        #print("out2.shape = ", out.shape)
        #out2.shape =  torch.Size([17, 1024, 16, 4, 4])
        
        out = self.avg_pool_n112(out)       
        #out3.shape =  torch.Size([17, 1024, 15, 1, 1])
        out = self.dropout_n112(out) 
        out = self.conv3d_0c_1x1_n112(out)
        #out5.shape =  torch.Size([17, 400, 1, 1, 1])
        
        out = out.squeeze(3)
        out = out.squeeze(3)
        out = out.mean(2)
        out_logits = out
        out = self.softmax_n112(out_logits)
